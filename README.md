...
- 🔍 **实时流量监控**: 基于libpcap的高性能网络包捕获，支持协议（TCP/UDP/ICMP）与端口维度明细
- 📈 **流量历史持久化**: 三层降采样归档（raw → 10分钟 → 1小时聚合），IP/协议/端口多维度趋势回溯30天
- 📊 **可视化界面**: React + TypeScript + ECharts 暗色仪表盘，协议分布/端口排行/趋势图/Top 榜/IP 详情
- 🧯 **策略引擎**: 限速（内核 hashlimit）、临时封禁、风险标记，多次触发自动升级封禁，内置防护模板
- 🛡️ **IP管理**: 支持单个IP或CIDR网段的封禁/允许管理，入站/出站方向可控，服务端分页与批量操作
- 🚦 **黑白名单操作**: 流量列表行内封禁/加白/解封、多选批量应用、IP 详情抽屉一站式管理
- 📁 **分组管理**: 支持IP分组管理，便于批量操作
- ⚡ **高性能**: 使用Go语言开发，支持高并发流量处理
- 🗄️ **存储**: 内置SQLite存储（WAL模式），无需外部数据库依赖
